 ...
 *
 * Nothing reads this at runtime - the slot is decided by each weapon's own `slot` field -
 * so it is a roster rather than a registry, and weapons that file themselves in from their
 * own module (the greatblade) add themselves so the roster stays true.
 */
export const MELEE_IDS = ['fist', 'knife', 'hammer'];
...
export const WEAPONS = {
  ...
  // further than the knife. The swing itself is twelve drawn frames (see hammerframes.js) and
  // takes a full second of that cooldown, so committing to it is the whole trade.
  hammer: {
    id: 'hammer', name: 'HAMMER', kind: 'melee', slot: 'melee',
 ...
 * which is the only way the hammer enters the match.
 */
export function randomDropId(rng) {
  const r = rng.next();
  if (r < 0.17) return 'hammer';
...
/**
 * The hammer. Long haft, a head that is deliberately too big for it - a drawn hammer, not a
 * tool - with the bit flaring out to one side so the swing has a direction you can read.
 */
function buildHammer(gl) {
  const IW = 1.7;
  ...
  // Head: one rectangular block, the same size at both ends. An axe tapers to a bit on one
  // side and is stubby on the other, which reads as lopsided from every angle except the one
  // it was drawn for; a hammer is the same slab whichever way round it is, so it stays
  // legible while it tumbles. Standing tall rather than spread wide - the height is what
  // makes it read as a maul instead of a mallet, and a 33cm-wide head at arm's length
  // covered most of the screen.
  pushOrientedBox(f, i, { pos: [0, 0.030, -0.222], size: [0.185, 0.340, 0.125], mat: MAT.METAL, inkWidth: IW });
  // Bands at each end, so the block has somewhere for the light to break.
  pushOrientedBox(f, i, { pos: [0.079, 0.030, -0.222], size: [0.040, 0.358, 0.142], mat: MAT.DARK, inkWidth: IW * 0.9 });
  pushOrientedBox(f, i, { pos: [-0.079, 0.030, -0.222], size: [0.040, 0.358, 0.142], mat: MAT.DARK, inkWidth: IW * 0.9 });
...
export const HOLD = {
  ...
  // Held further out, lower and further right than the knife, with the haft yawed the other
  // way so the head swings out of the middle of the screen instead of across it. A head this
  // size parked where a knife goes sits right on the crosshair, which you notice every
  // single time you try to line up a shot with the thing you are about to swap to.
  hammer:    { pos: [0.300, -0.300, -0.500], rot: [0.40, -0.35, 0.35], grip: [0, -0.01, 0.170], support: [0, -0.01, 0.300] },
};
